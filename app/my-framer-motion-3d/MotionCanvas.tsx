...
import type { Props, Camera, Dpr, ReconcilerRoot } from '@react-three/fiber';
import { createRoot, unmountComponentAtNode, events as createPointerEvents } from '@react-three/fiber';
import type { DimensionsState } from './MotionCanvasContext';
...
const calculateDpr = (dpr?: Dpr) =>
  Array.isArray(dpr) ? clamp(dpr[0], dpr[1], devicePixelRatio) : dpr || devicePixelRatio;
...
function CanvasComponent(
  { children, fallback, tabIndex, id, style, className, events, ...props }: MotionCanvasProps,
          ...
          <React.Suspense fallback={<Block set={setBlock} />}>
            ...
            >
              <MotionConfigContext.Provider value={configContext}>
                <MotionContext.Provider value={motionContext}>{children}</MotionContext.Provider>
